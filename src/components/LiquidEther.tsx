...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class MouseClass {
            ...
            init(container: HTMLElement) {
                this.container = container;
                // Listen globally so animation reacts even when covered by content
...
`;
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                const cursorSizeX = cursorSize * (cellScale as any).x;
                const cursorSizeY = cursorSize * (cellScale as any).y;
                const centerX = Math.min(
                    Math.max(Mouse.coords.x, -1 + cursorSizeX + (cellScale as any).x * 2),
                    1 - cursorSizeX - (cellScale as any).x * 2
                );
                const centerY = Math.min(
                    Math.max(Mouse.coords.y, -1 + cursorSizeY + (cellScale as any).y * 2),
                    1 - cursorSizeY - (cellScale as any).y * 2
                );
